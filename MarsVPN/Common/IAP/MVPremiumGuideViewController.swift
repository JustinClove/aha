...
//                control.isSelected = index == 1
        ...
        continueButton.top = previousView.bottom + 30
        ...
        label.top = previousView.bottom + 8
        view.addSubview(label)
        previousView = label

        view.addSubview(restoreLabel)
        restoreLabel.centerX = SCREEN_WIDTH/2
        restoreLabel.centerY = previousView.bottom + 16 + 9 + 2
...
//        freeLabel.centerY = titleLabel.centerY
    ...
    lazy var subPriceLabel: UILabel = {
        var label = UILabel()
        label.text = ""
        label.font = .regularMontserratFont(ofSize: 12)
        label.textColor = UIColor(hex: "#DDDDDE").withAlphaComponent(0.88)
        label.textAlignment = .right
        label.numberOfLines = 1
        label.sizeToFit()
        label.width = 200
...
//        var currencyPricePer = ""
        ...
        if basicDayCount >= 364 {
            ...
            //year pay mayby show month price
            perMonthString = "\(currencyStr)\(String(format: "%.2f", price/12))/Month"
        ...
        subPriceLabel.text = perMonthString
        ...
        if basicDayCount >= 364 {
            subtitleLabel.text = perMonthString
            subPriceLabel.text = perString
        }
        
        subtitleLabel.sizeToFit()
        subtitleLabel.centerY = self.height/2
        subtitleLabel.right = self.width - 29
        subPriceLabel.sizeToFit()
        subPriceLabel.left = subtitleLabel.left
        subPriceLabel.top = subtitleLabel.bottom + 2
    }
}
