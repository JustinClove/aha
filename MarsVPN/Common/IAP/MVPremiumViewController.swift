...
class MVPremiumViewController: LXBaseTableViewController {
    ...
    lazy var continueButton: UIButton = {
        let button = UIButton.createConsistent("Start free trial")
        
//        let str = ""\nStart free trial1\n"
//        let paraph = NSMutableParagraphStyle()
//        paraph.lineSpacing = 1
//        paraph.alignment = .center
//        var attributes = [NSAttributedString.Key.paragraphStyle: paraph, .font: UIFont.boldMontserratFont(ofSize: 16), .foregroundColor: UIColor.white]
//        var attributedStr = NSAttributedString(string: str, attributes: attributes)
//        let multAttr = NSMutableAttributedString(attributedString: attributedStr)
//        attributes = [.font: UIFont.regularMontserratFont(ofSize: 14), .foregroundColor: UIColor.white.alpha(0.98)]
//        attributedStr = NSAttributedString(string: "3 days free, then 12.99/month", attributes: attributes)
//        multAttr.append(attributedStr)
//        button.titleLabel?.numberOfLines = 2
//        button.setAttributedTitle(multAttr, for: .normal)
    ...
    override func viewDidLoad() {
        ...
        // 如果您需要在用户开始滑动关闭时执行一些操作，您可以监听 presentation controller 的变化
...
//                        make.bottom.lessThanOrEqualTo(0)
        ...
        continueButton.snp.makeConstraints { make in
            make.size.equalTo(self.continueButton.size)
            make.left.equalTo(24)
            make.top.equalTo(previousView.snp.bottom).offset(30)
        }
        ...
        label = UILabel()
        label.text = "3 days free, then 12.99/month"
        label.font = .regularMontserratFont(ofSize: 14)
        label.textAlignment = .center
        label.textColor = .init(hex: "#A3A6AF")
//        label.textColor = .white.alpha(0.98)
        label.width = SCREEN_WIDTH
        label.height = 13
        label.centerX = previousView.centerX
        label.top = previousView.bottom + 6
        view.addSubview(label)
        label.snp.makeConstraints { make in
    ...
    func updatePrice() {
        if let products = MVIAPManager.shared.products1, products.count > 1 {
            ...
            if dayCount > 0 {
                ...
                str = String(format: str, 3, 3, currencyPriceStr, currencyPriceStr)
            }
...
//        controls[index].isSelected = true
    ...
    func continueTapped() {
        let index = MVIAPManager.shared.productIdentifiers1.count - 1
        productTapped(index)
    }
...
extension MVPremiumViewController: UIAdaptivePresentationControllerDelegate {
    
     func presentationControllerWillDismiss(_ presentationController: UIPresentationController) {
        // 这里可以执行一些操作
        debugPrint("用户尝试滑动1关闭")
        self.complete?(false, nil)
