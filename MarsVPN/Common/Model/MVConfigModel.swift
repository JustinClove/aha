...
class MVConfigModel: FileManagerProtocol {
    ...
    private var _isShownRateUsCountdown: Int = 0
...
//#endif
    ...
    static var connectCount: Int {
        let value = self.current?._connectCount ?? 0
        return value
    }
    static func increaseConnectCount() {
        self.current?._connectCount += 1
        self.current?.saveToFile1()
    }
