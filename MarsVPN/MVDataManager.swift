...
class MVDataManager {
    ...
    public var freeLocations = [NodeModel]()
    public var vipLocations = [NodeModel]()
...
//                        node.remark = item["remark"].string
                ...
                shared.seperateLocationsToFreeAndVIP()
    ...
    func seperateLocationsToFreeAndVIP() {
        var arrayFree = [NodeModel]()
        var arrayVip = [NodeModel]()
        for node in self.locationList {
            
            // free
            if node.free == 1 {
                arrayFree.append(node)
                // vip
            } else {
                arrayVip.append(node)
            }
        }
        
        freeLocations = arrayFree
        vipLocations = arrayVip
    }
