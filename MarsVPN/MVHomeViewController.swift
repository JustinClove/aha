...
//        let vc = MVFreeLocationTableViewController()
        let vc = MVLocationViewController()
        vc.delegate = self
...
//            guard MVConfigModel.isVIP() else { return showPremium() }
            ...
            if !MVConfigModel.isVIP() && !MVConfigModel.shouldShownRateUs && MVConfigModel.connectCount >= 2 {
                DispatchQueue.main.asyncAfter(deadline: .now() + 0.5) { [unowned self] in
                    AdmobAdsManager.showInterstitial2(force:true, in: self) { [unowned self] _,_,_ in
                    }
                }
            }
...
extension MVHomeViewController: HVLocationSelectedProtocol {
    ...
    func connect(nodes: [NodeModel]?, adsFlag: Bool=false, premiumFlag: Bool=false,forceRefresh: Bool=false) {
...
//            if result {
                ...
                // no vip watch premium first
                if !MVConfigModel.isVIP() && !premiumFlag && MVConfigModel.connectCount >= 2 {
                    let vc = MVPremiumViewController()
                    vc.modalPresentationStyle = .fullScreen
                    vc.modalTransitionStyle = .coverVertical
                    self.present(vc, animated: true)
                    vc.complete = { [unowned vc] (result, errMsg) in
                        vc.dismiss(animated: true) {
                            self.connect(nodes: nodes, premiumFlag: true)
                        }
                    }
                    return
                }

                // no vip watch ads, first time no ads
                if !MVConfigModel.isVIP() && !adsFlag && MVConfigModel.connectCount >= 1 {
                    AdmobAdsManager.showInterstitial1(force:true, in: self) { [unowned self] _,_,_ in
                        self.connect(nodes: nodes, adsFlag: true, premiumFlag:true)
                    }
                    return
                }
...
//        }
        ...
        MVConfigModel.increaseConnectCount()
//        if let array = nodes {
